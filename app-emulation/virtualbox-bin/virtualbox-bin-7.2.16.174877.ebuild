...
inherit xdg-utils desktop pax-utils python-r1 udev unpacker

DESCRIPTION="Family of powerful x86 virtualization products for enterprise and home use"
HOMEPAGE="https://www.virtualbox.org/"
SRC_URI="
https://download.virtualbox.org/virtualbox/7.2.16/VirtualBox-7.2.16-174877-Linux_amd64.run -> virtualbox-bin-7.2.16.174877-Linux_amd64.run
https://download.virtualbox.org/virtualbox/7.2.16/Oracle_VirtualBox_Extension_Pack-7.2.16-174877.vbox-extpack -> virtualbox-bin-7.2.16.174877-oracle-extension_pack.tar.gz
sdk? ( https://download.virtualbox.org/virtualbox/7.2.16/VirtualBoxSDK-7.2.16-174877.zip -> virtualbox-bin-7.2.16.174877-sdk.zip )"
LICENSE="GPL-2 PUEL"
SLOT="0"
KEYWORDS="*"
IUSE="+additions doc headless python vboxwebsrv sdk"
...
"
PDEPEND="~app-emulation/virtualbox-modules-7.2.16.174877
...
src_unpack() {
	unpack_makeself virtualbox-bin-7.2.16.174877-Linux_amd64.run
	unpack ./VirtualBox.tar.bz2

	mkdir "${S}"/Oracle_VM_VirtualBox_Extension_Pack || die
	pushd "${S}"/Oracle_VM_VirtualBox_Extension_Pack &>/dev/null || die
	unpack virtualbox-bin-7.2.16.174877-oracle-extension_pack.tar.gz
	popd &>/dev/null || die

	if use sdk ; then
		unpack virtualbox-bin-7.2.16.174877-sdk.zip
	fi
}
src_configure() {
	:;
...
src_install() {
	...
	use python && doins VBoxPython3.so
	...
	if use headless ; then
		rm -rf VirtualBox{,VM} VBoxKeyboard.so VirtualBoxVM.so UICommon.so
	fi
